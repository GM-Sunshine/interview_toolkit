...
from src.utils.config import (API_TYPE, DEFAULT_API_TIMEOUT,
                            DEFAULT_MAX_TOKENS, DEFAULT_TEMPERATURE,
                            OLLAMA_BASE_URL, OLLAMA_MODEL, OPENAI_API_KEY,
                            OPENAI_MODEL, validate_config)
...
openai_limiter = RateLimiter(tokens_per_second=3/60, max_tokens=3)  # 3 tokens per minute
# Ollama has no official rate limit, but we'll be conservative
ollama_limiter = RateLimiter(tokens_per_second=10, max_tokens=20)  # 10 tokens per second
...
def retry_with_exponential_backoff(
    ...
    for retry in range(max_retries + 1):
        try:
            ...
            print(f"Retry {retry + 1}/{max_retries} after {current_delay:.2f}s due to: {str(e)}")
        ...
        def _generate_completion():
            ...
            try:
                ...
                if debug:
                    ...
                    print("This may take a while depending on the model size and complexity of the request.")
                ...
                if response.status_code != 200:
                    error_msg = f"Ollama server returned error status: {response.status_code}"
                    if debug:
                        print(f"\n{error_msg}")
                ...
                try:
                    ...
                    raise ParseError(f"Invalid JSON response from Ollama server: {str(e)}")
