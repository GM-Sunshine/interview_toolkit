...
class TextRenderer:
    ...
    def draw_text_with_highlights(
        self, c, text, x, y, width, font_name, font_size, text_color
    ):
        ...
        if "```" in text:
            ...
            for part in parts:
                ...
                if part_type == "code_block":
                    ...
                    current_y = (
                        line_y - padding_bottom / 2
                    )  # Extra space after code block to ensure separation

                else:
                    # Process regular text, but check for inline code within it
                    if "`" in part_text:
                        # Process text with inline code blocks
                        self._draw_text_with_inline_code(
                            c,
                            part_text,
                            x,
                            current_y,
                        ...
                        wrapped_lines = self._wrap_text(
                            part_text, c, font_name, font_size, width
                        )
                        for line in wrapped_lines:
                            c.drawString(x, current_y, line)
                            current_y -= (
                                font_size * 1.5
                            )  # Increased line spacing to match code blocks

        # Handle case where we only have inline code blocks (no triple backticks)
        elif has_inline_code:
            # Process text with inline code blocks
            current_y = self._draw_text_with_inline_code(
                c, text, x, current_y, width, font_name, font_size, text_color
            )
    ...
    def _identify_php_syntax(self, text):
        ...
        # PHP literals/constants
            ...
            text in ("true", "false", "null", "TRUE", "FALSE", "NULL") or text.isdigit()
        ):
    ...
    def _draw_text_with_inline_code(
        self, c, text, x, y, width, font_name, font_size, text_color
    ):
                    ...
                    if code_width > max_line_width:
                        ...
                        while remaining_code:
                            ...
                            for i in range(1, len(remaining_code) + 1):
                                test_width = c.stringWidth(
                                    remaining_code[:i], "Courier", font_size
                                )
        ...
        for line in wrapped_segments:
            ...
            current_y -= (
                font_size * 1.5
            )  # Increased line spacing to avoid overlap with code blocks
    ...
    def _wrap_text(self, text, canvas, font_name, font_size, max_width):
        ...
        for word in words:
            ...
            if current_line:
                word_with_space = " " + word
                test_width = current_width + canvas.stringWidth(
                    word_with_space, font_name, font_size
                )
            else:
                word_with_space = word
                ...
                if current_line:
                    ...
                    while remaining:
                        ...
                        for i in range(1, len(remaining) + 1):
                            if (
                                canvas.stringWidth(remaining[:i], font_name, font_size)
                                > max_width
                            ):
                                chars_fit = i - 1
                                break
