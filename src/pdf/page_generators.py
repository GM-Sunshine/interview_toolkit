...
def create_cover_page(c, pdf_gen):
    ...
    add_subtle_pattern(c, 0, 0, width, height, colors.get("primary", (0, 0, 0.8)), opacity=0.1)
    ...
    for logo_file in os.listdir("logos"):
        ...
        logo_name = os.path.splitext(logo_file)[0].lower().replace(" ", "").replace("-", "").replace("_", "")
        ...
        if logo_name in normalized_title or normalized_title in logo_name:
            try:
                ...
                # Draw logo
                c.drawImage(f"logos/{logo_file}", logo_x, logo_y, width=logo_width, height=logo_height, mask='auto')
                c.restoreState()
                logo_found = True
                break
            except Exception:
    ...
    if not logo_found:
        try:
            ...
            if os.path.exists(gm_logo_path):
                ...
                # Draw logo
                c.drawImage(gm_logo_path, logo_x, logo_y, width=logo_width, height=logo_height, mask='auto')
                c.restoreState()
        except Exception:
            # Silently handle logo errors, not critical for PDF generation
            pass
    ...
    subtitle_width = c.stringWidth(subtitle, fonts["content_font"], fonts["subtitle_size"] * 1.2)
    ...
    try:
        ...
        for word in quote.split():
            ...
            if c.stringWidth(test_line, fonts["content_font"], fonts["content_size"]) < max_quote_width:
        ...
        for i, line in enumerate(quote_lines):
            line_width = c.stringWidth(line, fonts["content_font"], fonts["content_size"])
        ...
        author_width = c.stringWidth(author_text, fonts["content_font"], fonts["content_size"])
...
def create_qa_page(canvas, pdf_gen, question_data):
    ...
    if is_dark_theme:
        ...
        background_color = colors.get("background", (1, 1, 1))  # Default to white if not found
    ...
    question_has_code = "```" in question_data["question"] or "`" in question_data["question"]
    ...
    inline_code = question_data["question"].count("`") - (code_blocks * 2)  # Each code block has 2 sets of ```
    
    # If question has code blocks, allocate more space
    if code_blocks > 0:
        min_question_space = fonts["content_size"] * 1.2 * (question_lines + 2)
    else:
        min_question_space = fonts["content_size"] * 1.2 * max(5, question_lines + (inline_code // 4))
    ...
    if is_dark_theme:
        ...
        canvas.setFillColorRGB(*primary_color)  # Use primary color for footer in light mode
    canvas.drawCentredString(width / 2, margin / 2, f"{pdf_gen.title} • Question & Answer")
...
def create_milestone_page(c, pdf_gen, progress_percentage):
    ...
    progress_message = pdf_gen["progress_slides"].get(progress_percentage, f"{progress_percentage}% Complete")
    ...
    c.drawString(page_width / 2 - message_width / 2, page_height / 2 - 24, progress_message)
...
def create_summary_page(c, pdf_gen, questions_data):
    ...
    try:
        # Draw background
        bg_color = hex_to_rgb(
            pdf_gen["colors"]["background"]
        )
        r, g, b = bg_color
        c.setFillColorRGB(r, g, b)
        c.rect(0, 0, pdf_gen["page_width"], pdf_gen["page_height"], fill=True, stroke=False)
        ...
        c.drawRightString(
            pdf_gen["page_width"] - 1 * cm, pdf_gen["page_height"] - 1.5 * cm, pdf_gen["title"]
        )
        ...
        for idx, qa in enumerate(questions_data):
            ...
            c.drawString(
                2 * cm, y_position, f"{idx + 1}. {question}"
            )
            y_position -= 0.8 * cm
...
def create_progress_slide(c, pdf_gen, current_milestone, total_questions):
    ...
    if hasattr(pdf_gen, 'progress_slides') and pdf_gen.progress_slides:
        ...
        if isinstance(milestone_data, dict) and 'messages' in milestone_data:
            ...
            milestone_message = str(milestone_data) if milestone_data else f"You've completed {percentage}% of the questions!"
    else:
        # Default message if no progress_slides dictionary
        milestone_message = f"You've completed {percentage}% of the questions!"

    # Extract parameters
    width, height = pdf_gen.page_size if hasattr(pdf_gen, 'page_size') else (letter[0], letter[1])
    margin = pdf_gen.margin if hasattr(pdf_gen, 'margin') else 0.75 * inch
    colors = pdf_gen.colors if hasattr(pdf_gen, 'colors') else {'primary': (0, 0, 0.8), 'secondary': (0, 0, 0.6)}
    fonts = pdf_gen.fonts if hasattr(pdf_gen, 'fonts') else {'title_font': 'Helvetica-Bold', 'content_font': 'Helvetica'}
    ...
    text_width = c.stringWidth(percentage_text, fonts.get("title_font", "Helvetica-Bold"), font_size)
    ...
    for word in words:
        test_line = current_line + " " + word if current_line else word
        if c.stringWidth(test_line, fonts.get("title_font", "Helvetica-Bold"), 24) < max_width:
            current_line = test_line
        else:
            if current_line:
        ...
        if isinstance(milestone_data, dict) and 'quote' in milestone_data:
            ...
            if quote and len(quote) == 2:
                try:
                    ...
                    # Draw quote
                    quote_y = height * 0.2
                    c.setFont(fonts.get("content_font", "Helvetica"), fonts.get("content_size", 12))
                    c.setFillColorRGB(*colors.get("background", (1, 1, 1)))
                    ...
                    for word in quote_text.split():
                        test_line = current_line + " " + word if current_line else word
                        if c.stringWidth(test_line, fonts.get("content_font", "Helvetica"), fonts.get("content_size", 12)) < max_quote_width:
                            current_line = test_line
                        else:
                            quote_lines.append(current_line)
                            current_line = word
                    ...
                    # Draw each line of the quote
                    for i, line in enumerate(quote_lines):
                        line_width = c.stringWidth(line, fonts.get("content_font", "Helvetica"), fonts.get("content_size", 12))
                    ...
                    c.setFont(fonts.get("content_font", "Helvetica"), fonts.get("content_size", 12))
                    author_text = f"— {author}"
                    author_width = c.stringWidth(author_text, fonts.get("content_font", "Helvetica"), fonts.get("content_size", 12))
                    c.drawString(width/2 - author_width/2, author_y, author_text)
                except Exception:
                    # Silently handle quote errors, not critical for PDF generation
                    pass
...
def create_ending_page(c, pdf_gen, completion_message=None):
    ...
    try:
        ...
        add_subtle_pattern(c, 0, 0, width, height, colors.get("primary", (0, 0, 0.8)), opacity=0.15)
        ...
        if qrcode_available:
            try:
                ...
                qr_y = height - logo_height - 60 - qr_size  # Below logo with some spacing
                ...
                url_width = c.stringWidth(url_text, fonts.get("content_font", "Helvetica"), 9)
        ...
        c.setFillColorRGB(*colors.get("background", (1, 1, 1)))
        c.setFont(fonts.get("title_font", "Helvetica-Bold"), fonts.get("title_size", 24) * 1.5)
        c.drawCentredString(
            width / 2, height * 0.6, "Thank You!"
        )
        ...
        c.setFillColorRGB(*colors.get("background", (1, 1, 1)))
        c.setFont(fonts.get("content_font", "Helvetica"), fonts.get("subtitle_size", 18))
        ...
        for word in words:
            test_line = current_line + " " + word if current_line else word
            if c.stringWidth(test_line, fonts.get("content_font", "Helvetica"), fonts.get("subtitle_size", 18)) < max_width:
                current_line = test_line
            else:
        ...
        c.drawCentredString(
            width / 2,
            height * 0.1,
            "© Interview Toolkit - For personal use only"
        )
