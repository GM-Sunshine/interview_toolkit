...
from .page_generators import (create_cover_page, create_final_page,
                             create_milestone_page, create_qa_page, create_progress_slide, create_ending_page)
...
class QuestionNumbering(Flowable):
    ...
    def draw(self):
        ...
        number_width = self.canv.stringWidth(number_text, "Helvetica-Bold", self.size * 0.6)
        self.canv.drawString(radius - number_width/2, radius - self.size * 0.2, number_text)
...
def create_cover_page(canvas, pdf_gen):
    ...
    subtitle_width = canvas.stringWidth(subtitle, fonts["content_font"], fonts["subtitle_size"])
...
def create_qa_page(canvas, pdf_gen, question_data):
    ...
    background_color = colors.get("background", (1, 1, 1))  # Get background from colors or default to white
...
def create_pdf(questions, output_file, title=None, color_scheme="blue"):
                ...
                try:
                    ...
                    if key == "primary":
                        ...
                        if color_scheme == "dark":
                            colors[key] = (0.03, 0.03, 0.03)  # #080808 very dark background
                        else:
                            colors[key] = (1.0, 1.0, 1.0)  # White for light themes
                    elif key == "text":
                        # For dark theme use light text, for light themes use dark text
                        if color_scheme == "dark":
                            colors[key] = (0.9, 0.9, 0.9)  # Light gray for dark theme (changed from pure white)
                        else:
    ...
    try:
        ...
        # Create a final page
        canvas.setPageSize(page_size)
        create_ending_page(canvas, pdf_gen, f"You've completed all {len(questions)} questions!")
...
class PDFGenerator:
    ...
    def __init__(self, page_size, margin, colors, fonts, title, progress_slides, color_scheme=None):
        """
