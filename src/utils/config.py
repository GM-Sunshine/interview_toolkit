...
DEFAULT_API_TIMEOUT = int(os.environ.get("DEFAULT_API_TIMEOUT", "900"))  # 15 minutes timeout
...
def _validate_api_type(api_type: str, errors: Dict[str, str]) -> None:
    ...
    if api_type not in ["openai", "ollama"]:
        errors["API_TYPE"] = f"Invalid API type: {api_type}. Must be 'openai' or 'ollama'"

def _validate_openai_config(api_type: str, api_key: str, errors: Dict[str, str]) -> None:
    """Validate OpenAI API configuration."""
    if api_type == "openai" and not api_key:
        errors["OPENAI_API_KEY"] = "OpenAI API key is required when API_TYPE is 'openai'"

def _validate_temperature(temperature: float, errors: Dict[str, str]) -> None:
    """Validate temperature."""
    if temperature < 0 or temperature > 1:
        errors["DEFAULT_TEMPERATURE"] = f"Invalid temperature: {temperature}. Must be between 0 and 1"

def _validate_max_tokens(max_tokens: int, errors: Dict[str, str]) -> None:
    """Validate max tokens."""
    if max_tokens <= 0:
        errors["DEFAULT_MAX_TOKENS"] = f"Invalid max tokens: {max_tokens}. Must be greater than 0"

def _validate_api_timeout(api_timeout: int, errors: Dict[str, str]) -> None:
    """Validate API timeout."""
    if api_timeout <= 0:
        errors["DEFAULT_API_TIMEOUT"] = f"Invalid API timeout: {api_timeout}. Must be greater than 0"
