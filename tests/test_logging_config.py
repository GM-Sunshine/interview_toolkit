...
def test_setup_logging_creates_directory(
    ...
    mock_file_formatter = MockFormatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
...
def test_logging_rotation_settings():
    ...
    assert "maxBytes=1024 * 1024" in source_code, "MaxBytes setting not found in source code"
    assert "backupCount=5" in source_code, "BackupCount setting not found in source code"
    assert "interview_toolkit.log" in source_code, "Log filename not found in source code"
