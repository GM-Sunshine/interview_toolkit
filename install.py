...
def check_python_version() -> bool:
    ...
    if major < required_major or (major == required_major and minor < required_minor):
        print_error(
            f"Python {required_major}.{required_minor}+ is required, but found {major}.{minor}"
        )
        return False
...
def setup_virtual_environment() -> bool:
    ...
    if os.path.exists(venv_path):
        response = input(
            f"Virtual environment already exists at {venv_path}. Recreate? (y/n): "
        )
        if response.lower() == "y":
            try:
...
def setup_environment_file() -> bool:
    ...
    if os.path.exists(env_path):
        response = input(
            f"Environment file already exists at {env_path}. Overwrite? (y/n): "
        )
        if response.lower() != "y":
    ...
    try:
        ...
        print_warning(
            f"Please edit {env_path} to set your API keys and other configuration"
        )
...
def print_next_steps() -> None:
    ...
    print_colored(
        "1. Edit the .env file to set your API keys and other configuration",
        Colors.BLUE,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-env", action="store_true", help="Skip environment file setup"
    )
